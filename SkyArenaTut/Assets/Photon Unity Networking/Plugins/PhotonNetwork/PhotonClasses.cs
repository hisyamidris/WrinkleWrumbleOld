...
public enum PhotonTargets
{
    ...
    OthersBuffered,
    /// <summary>Sends the RPC to everyone through the server. Locally, the execution is delayed by the roundtrip time (Ping).</summary>
    /// <remarks>Benefit: The server's order of sending the RPCs is the same on all clients.</remarks>
    AllViaServer,
    /// <summary>Sends the RPC to everyone through the server which buffers it for new players. Locally, the execution is delayed by the roundtrip time (Ping).</summary>
    /// <remarks>Benefit: The server's order of sending the RPCs is the same on all clients.</remarks>
